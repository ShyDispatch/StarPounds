...
function update(dt)
  ...
  erchiusCount = status.uniqueStatusEffectActive("bottlederchiusbloat") and 0 or erchiusCount
  ...
  if monsterPosition then
    ...
    self.bps = math.floor(math.min((erchiusCount/5)^(2/3), 500) * 100 + 0.5)/100 * effectDistanceRatio
    sb.setLogMap("bps", self.bps)
