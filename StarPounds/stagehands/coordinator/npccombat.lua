...
function attackPositionsAlongGround(minRange, maxRange, targetPosition, bounds)
  local bounds = bounds or self.npcBounds
  local positions = {}
  ...
  for range = math.floor(minRange), math.ceil(maxRange) do
    ...
    for _,dir in pairs({1,-1}) do
      local groundPosition = findGroundAttackPosition(vec2.add(targetPosition, {dir * range, 0}), -range, range, targetPosition, bounds)
...
function attackPositionsInRange(maxRange, minRange, center, bounds)
  local bounds = bounds or self.npcBounds
  local positions = {}
    ...
    for i = 0, maxSteps do
      ...
      for _,xDir in ipairs({1, -1}) do
        for _,yDir in ipairs({1, -1}) do
          ...
          position[2] = math.ceil(position[2]) - ((bounds[2] or self.npcBounds[2]) % 1)
          if validAttackPosition(position, bounds, true) and not world.lineTileCollision(position, vec2.add(center, {0, -1})) then
